...
public class WrapperService {
	 ...
	 * 
	 * *** 중요 ***
	 * - Java에서는 Boxing, Unboxing을 자동으로 수행되도록 구현되어 있음(jdk 11부터 강제로 못 쓰게 함)
	...
	public void test1() {
		...
		// deprecated : 사용되지 않음(=이제 이거 안 쓴다~)
		// -> 컴파일 오류는 나지 않지만 버전이 올라가면 삭제될 수도 있으니 쓰지 말아라!
		Integer wrap1 = new Integer(num1);
		
		// 기본 자료형 int -> Wrapper Class 객체 Integer로 변경 후
		// Object를 전달 받는(모든 객체를 전달 받는) 메서드 printValue의 전달 인자로 작성
		printValue(wrap1);
	}
	...
	public void test3() {
		// 정수형, 실수형 공통 필드
		System.out.println("int는 몇 바이트? " + Integer.BYTES); // 4
		System.out.println("int는 몇 비트? "   + Integer.SIZE); // 32
		System.out.println("int 최대값 : " + Integer.MAX_VALUE); // 2147483647
		System.out.println("int 최대값 : " + Integer.MIN_VALUE); // -2147483648
		System.out.println("----------------------------------");
		
		System.out.println("double은 몇 바이트? " + Double.BYTES); // 8
		System.out.println("double은 몇 비트? "   + Double.SIZE); // 64
		System.out.println("double 최대값 : " + Double.MAX_VALUE); // 1.7976931348623157E308
		System.out.println("double 최대값 : " + Double.MIN_VALUE); // 4.9E-324

		// 실수형만 사용 가능한 필드
		System.out.println("----------------------------------");
		System.out.println(Double.NaN); // 숫자 아님
		System.out.println(Double.POSITIVE_INFINITY); // + 무한 Infinity
		System.out.println(Double.NEGATIVE_INFINITY); // - 무한 -Infinity
