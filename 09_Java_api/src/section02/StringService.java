...
public class StringService {
	 ...
	 *  ex) String s1 = "hello";
	 *  -> heap 메모리 영역에 존재하는
	 *     String Constant(상수) Pool(웅덩이, 고여있는)에 객체 생성 
	 *  
	 * 
	 * 2) new 연산자를 이용하여 생성 
	...
	public void test2() {
		...
		// 같은 값을 지닌 String 객체를 이터럴을 이용해 생성
		String s2 = "hello";
		...
		System.out.println("[필드 값 비교] s1.equlas(s2) : " + s1.equals(s2)); // true
		System.out.println("[필드 값 비교] s1.equlas(s3) : " + s1.equals(s3)); // true
	}
	
	
	
	/* [StringBuilder-문자열을 짓다 / StringBuffer-문자열을 쌓다]
	 ...
	 * 
	 * - 문자열의 추가/수정/삭제가 발생해도
	 *   별도 String 객체 생성이 없어서 메모리 효율이 좋다!
	 *   
	 * - StringBuilder : Tread Safe 미지원(비동기, 빠름)
	 * - StringBuffer  : Tread Safe 지원(동기, 느림)
	 */
	public void test3() {
		// StringBuilder 객체 생성
		StringBuilder sb = new StringBuilder();
		
		System.out.println("sb 크기 : " + sb.capacity()); // capacity : 용량/크기
		System.out.println("sb 주소 : " + System.identityHashCode(sb));
		// -> sb 크기 : 16 / sb 주소 : 424058530
		...
		/* toString() : StringBuilder 객체에 저장된 값을 String으로 반환 */
		System.out.println("--------------------------------");
		...
		/* insert(int offset, String s) : offset(=index) 위치에 s 삽입 */
		sb.insert(0, "가나다라");
		System.out.println("--------------------------------");
		System.out.println("sb.toString() : " + sb.toString());
		System.out.println("sb 크기 : " + sb.capacity()); // capacity : 용량/크기
