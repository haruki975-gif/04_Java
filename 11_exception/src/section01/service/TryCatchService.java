...
public class TryCatchService {
	...
	public void test2() {
		...
		try {
			...
			// catch는 여러번 작성 가능!!!!
			// -> try에서 예외 발생 시 던져진 예외를 잡아서 처리할 수 있는 catch문을
			//    위에서부터 순서대로 탐색 -> 알맞은 catch문에서 예외 처리 수행
	...
	public void test4() {
		 ...
		 * 자식 타입 예외까지 모두 잡기 때문에 뒤에 있는 자식 catch문이 절대로 실행될 수 없다!!
		 * */ 
		//	catch(RuntimeException e) {
		...
		// -> RuntimeException catch 블록이 먼저 처리되고 있다
		catch(InputMismatchException e) {
			System.out.println("정수만 입력해 주세요.");
		}
