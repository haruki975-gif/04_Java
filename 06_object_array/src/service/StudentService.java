...
public class StudentService {
	...
	public StudentService() {
		...
		students[1] = new StudentDTO("20250331", "철수", '남');
		students[2] = new StudentDTO("20240124", "애영", '여');
	 ...
	 * (students에 저장된 주소가 복사되어 반환됨 == 얕은 복사)
	 * @return students
	 */
	...
	public StudentDTO selectIndex(int index) {
		...
		// java.lang.ArrayIndexOutOfBoundsException : 배열 인덱스 범위 초과 예외 
		// Index 6 out of bounds for length 5
		
		// index가 배열 범위를 벗어난 경우
		if(index < 0 || index >= students.length) { // students.length = 5
			return null;
		}
	...
	public StudentDTO selectName(String targetName) {
		...
		for(StudentDTO std : students) {
			...
			// (객체가 가지고 있는 실제 값을 비교(주소를 비교하는 게 아님))
			...
			if(std.getName().equals(targetName)) {
				...
				// std.getName()과 targetName에 저장된 값(주소)이 같을 경우
				// - 참조형은 주소를 저장하고 있다!
	...
	/** 전달 받은 인덱스가 students 범위 내 인덱스가 맞는지
	...
	public String deleteStudent(int index) {
		...
		// 1) students 배열에서 입력받은 index 번째 요소를 null로 바꿈
		students[index] = null;
		
		// 2) 삭제된 인덱스 뒤에 요소를 하나씩 당겨오기
		for(int i = index; i < students.length-1; i++) { // i=1,2,3
			students[i] = students[i+1];
		}
		
		// 마지막 index에 null 대입 (제일 마지막을 비워둠)
