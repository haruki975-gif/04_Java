...
public class TestRun {
	public static void main(String[] args) {
		...
		// p1이 호출 가능한 메서드
		// - Parent의 메서드 + Object에게 상속 받은 메서드
		 ...
		 * 
		 * * 다형성 - 업캐스팅(Up Casting)
		 * - 자식 객체의 형태를 부모 객체의 형태로 변환
		 * - '부모' 참조 변수 = new '자식' 객체;
		 *   -> 자식 객체가 부모 객체 형태로 변한 것처럼 보임
		 *   
		 *   
		 * * 다형성 - 다운캐스팅(Down Casting)
		 * - 업캐스팅 상태에서 (부모 참조 변수 = 자식 객체)
		 * 	 참조 변수의 자료형을 자식 타입으로 강제 형변환하는 것
		 */
		...
		Parent p2 = new Child("김", 1500, "소나타");
		
		// p2가 호출 가능한 메서드
		// - Parent, Object 메서드만 호출 가능
		// - Child 객체이지만 Parent로 변경되어
		//   Child 메서드 인식 불가(=호출 불가능)
