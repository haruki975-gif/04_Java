...
public class PolymorphismService {
	 ...
	 * 
	 * - "참조변수"가 참조하는 객체가
	 *   "클래스명"으로 만들어진 객체가 맞을 경우 true, 아니면 false
	 * 
	 * - 상속 관계도 파악이 가능
	 *   (자식 객체 내부의 부모 객체도 인식)
	 */
...
//		else if(obj instanceof Object) {
			...
			// -> 모든 클래스 객체는 Object의 자식이기 때문에 else에 모두 걸린다.
	...
	public void test1() {
		...
		Parent parent = new Parent("신", 500);
		Child child   = new Child("백", 200, "모닝");
	...
	public Object getInstance(int num) {
		...
		// if, for, while의
		// {} 내부 코드가 한 줄인 경우 {} 생략 가능 (메서드, 클래스는 X)
	 ...
	 * - 왜 사용?
	 *   1) 다운캐스팅의 번거로움을 없앰
	 *   2) 재정의 된 메서드(조금 더 효율적 또는 알맞는 기능)을 수행
	 *   
	...
	// 바인딩 확인
	public void test3() {
		Parent p1 = new Parent("신", 10); // 부모 = 부모
		Child c1 = new Child("백", 200, "카니발"); // 자식 = 자식
		
		Parent p2 = new Child("서", 30, "에쿠스"); // 부모 = 자식(업캐스팅)
	...
	public void test5() {
		...
		// 업캐스팅
		Parent p = new Child("김", 120, "에쿠스");
		test6(p);
		
	}
	
	// 전달 받은 객체를 String 타입으로 다운캐스팅(강제 형변환)
	public void test6(Object obj) {
		
		/* ClassCastException 발생 
		 * - 참조 변수의 강제 형변환(다운캐스팅) 시
		 *   참조하는 객체가
		 *   변환하려는 타입이 아니거나 상속 관계도 아니면
		 *   형변환 불가(ClassCastException) 발생
		 * */
		
		// 참조하는 객체가 String 타입일 경우에만 형변환(다운캐스팅)
