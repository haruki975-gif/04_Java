...
public class FunctionTest {
	public static void main(String[] args) {
		...
		/* Function 계열
		 * - 매개 변수를 받아서 다른 타입으로 변환(가공)하여 반환하는 함수형 인터페이스
		 * - R applyXXX(T) 메서드 제공
		 */
		
		/* Function<T, R> : 매개 변수로 T 전달, R 타입으로 반환 */
		Function<String, Integer> strLength = str -> str.length();
		System.out.println(strLength.apply("Function의 글자수")); // 13글자
		...
		IntToDoubleFunction randomNumber =  range -> Math.random() * range; // 0 이상 range미만 실수 반환
